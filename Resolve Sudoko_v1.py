...
qtd_por_quadrado = []  # Quantidade de cada numero por quadrado
nqfq = []  # Numeros que falta no quadrado


qtd_por_horizontal = []  # Quantidade de cada numero por horizontal
nqfh = []  # Numeros que falta na Horizonta.


qtd_por_vertical = []  # Quantidade de cada numero por vertica
nqfv = []  # Numeros que falta na vertical
...
def analisa_matriz():
    ...
    for i in range(9):
        ...
        if cont == 0:
            ...
            print(mat[i], f'linha {i} contem {cont} zeros')


def analisa_quadrados():
    print()
    print('Quadrados')
    for i in range(9):
        cont = 0
        ...
        if cont == 0:
            ...
            print(quadrados[i], f'quadrado {i} contem {cont} zeros')
            ...
            for k in range(6, 9):
                if i not in quadrados[k]:
                    ...
                    if casa_preenchida < 2:
                        ...
                        if len(pata) == 1:
                            mat[linha_que_não_tem][((k-6) * 3) + pata[0]] = i
                    if casa_preenchida == 2:
                        for m in mat[linha_que_não_tem][(k-6) * 3: ((k-6) * 3) + 3]:
                            if m == X:
                                posicao = (
                                    k-6) * 3 + mat[linha_que_não_tem][(k-6) * 3: ((k-6) * 3) + 3].index(X)
                ...
                if i not in quadrados[quadrado]:
                    ...
                    if casa_preenchida == 2:
                        for m in mat[linha_que_não_tem][quadrado * 3: (quadrado+1) * 3]:
                            if m == X:
                                posicao = (
                                    quadrado * 3 + mat[linha_que_não_tem][quadrado * 3: (quadrado+1) * 3].index(X))
                                mat[linha_que_não_tem][posicao] = i
    ...
    for i in qtd_por_horizontal:
        if i == 8:
            ...
            mat[qtd_por_horizontal.index(i)][mat[qtd_por_horizontal.index(
                i)].index(X)] = nqfh[qtd_por_horizontal.index(i)][0]
            ...
            for p in quadrados[::3]:
                if num not in p:
                    ...
                    if cont == 2:
                        for i in range(3):
                            if mat[quadrados.index(p)+i][ntem_num[0]] == 0:
                                mat[quadrados.index(p)+i][ntem_num[0]] = num
            ...
            for p in quadrados[1::3]:  # Entra dentro do quadrado
                if num not in p:
                    ...
                    if cont == 2:
                        for i in range(3):
                            if mat[(quadrados.index(p)+i)-1][ntem_num[0]] == 0:
                                mat[(quadrados.index(p)+i)-1][ntem_num[0]] = num
            ...
            for p in quadrados[2::3]:  # Entra dentro do quadrado
                if num not in p:
                    ...
                    if cont == 2:
                        for i in range(3):
                            if mat[(quadrados.index(p)+i)-2][ntem_num[0]] == 0:
                                mat[(quadrados.index(p)+i)-2][ntem_num[0]] = num
...
while geral[0][0] < 81:
    acabou()
    print('Quantidade geral: ', geral[0][0])

    ultimo.append(geral[0][0])
    if ultimo[-2:-1] == ultimo[-1:]:
        break
